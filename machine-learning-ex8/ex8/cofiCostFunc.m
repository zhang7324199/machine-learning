...
%for i=1:num_movies    
 % for j=1:num_users
  %  if R(i, j)==1   %判断用户是否对电影评分
    %J += (Theta(j,:)*X(i,:)'-Y(i,j))^2/2;
    %for k=1:num_features   %循环梯度  k为第几个特征
...
%Theta:944*10    X: 1682*10
J = sum(sum((X*Theta'-Y).^2.*R/2)) + (lambda/2*sum(sum(X.^2)))+(lambda/2*sum(sum(Theta.^2)));
